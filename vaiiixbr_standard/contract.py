...
BUY_CONFIRMED_THRESHOLD = 0.78
BUY_WATCHLIST_THRESHOLD = 0.58
...
def _build_entry_gate(decision: Dict[str, Any], metrics: Dict[str, Any], news: Dict[str, Any]) -> Dict[str, Any]:
    ...
    price_bias = str(news.get("price_bias", "NEUTRAL")).upper()
    confidence_hint = _safe_float(news.get("confidence_hint", 0.0))
    ...
    elif status in {"NEUTRAL", "UNKNOWN", "UNINITIALIZED"}:
        blockers.append("o sistema está neutro ou ainda não foi inicializado")

    if final_confidence >= BUY_CONFIRMED_THRESHOLD:
        positives.append(f"confiança alta ({final_confidence:.2%})")
    elif final_confidence >= BUY_WATCHLIST_THRESHOLD:
    ...
    if price_bias == "UP_BIAS":
        ...
        blockers.append("noticiário neutro ou ausente")

    if confidence_hint < 0:
        blockers.append("ajuste de confiança por notícias está negativo")

    if _safe_bool(metrics.get("position_open"), False):
    ...
    if cooldown_remaining > 0:
        blockers.append(f"cooldown ativo em {cooldown_remaining} barras")

    ready = (
        final_signal == "BUY"
        and status == "CONFIRMED"
        and final_confidence >= BUY_CONFIRMED_THRESHOLD
        and not _safe_bool(metrics.get("position_open"), False)
        and cooldown_remaining == 0
    )

    if ready:
        verdict = "ENTRADA CONFIRMADA"
    elif final_signal == "BUY" and final_confidence >= BUY_WATCHLIST_THRESHOLD:
...
def normalize_trade_result(
    result: Dict[str, Any],
    latest_news: Dict[str, Any] | None = None,
    ...
    normalized = {
        ...
        "metrics": {
            ...
            "position_open": _safe_bool(metrics.get("position_open", False), False),
